...
def get_call_from_expr(func_expr):
    ...
    if func_expr["nodeType"] == "Expr_FuncCall":
        ...
        func_expr["nodeType"] == "Expr_BinaryOp_Concat"
        and func_expr["right"]["nodeType"] == "Expr_FuncCall"
    ):
    ...
    for el in walk(tree_el):
        ...
        if el["nodeType"] == "Stmt_Use":
            for use in el["uses"]:
                ...
                variables.append(
                    Variable(token, points_to=owner_token, line_number=lineno(el))
                )
...
def run_ast_parser(filename):
    ...
    script_loc = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "get_ast.php"
    )
...
class PHP(BaseLanguage):
    ...
    def make_nodes(tree, parent, source_code=None):
        ...
        is_constructor = (
            token == "__construct" and parent.group_type == GROUP_TYPE.CLASS
        )

        tree_body = tree["stmts"]
        subgroup_trees, subnode_trees, this_scope_body = PHP.separate_namespaces(
            tree_body
        )
        ...
        if (
            parent.group_type == GROUP_TYPE.CLASS
            and parent.parent.group_type == GROUP_TYPE.NAMESPACE
        ):
    ...
    def make_class_group(tree, parent, source_code=None):
        ...
        if group_type == GROUP_TYPE.NAMESPACE:
            ...
            for node in class_group.nodes:
                node.variables += [
                    Variable(n.token, n, line_number=n.line_number)
